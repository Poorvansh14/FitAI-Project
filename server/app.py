...
import sqlite3, bcrypt, os, datetime, re
...
if not api_key:
    print("⚠️ Warning: GEMINI_API_KEY not found. The API will fail until set in Railway variables.")
else:
...
@app.route('/')
def home():
    return "✅ FitAI backend is live on Railway!", 200
...
def generate_diet():
    try:
        ...
        # Forbidden items for vegans/vegetarians
        forbidden = []
        if pref == "vegetarian":
